...
export async function GET(
  ...
  try {
    ...
    const productBrand = await getProductBrandById(id)

    return Response.json(productBrand, { status: 200 })
  } catch (error) {
...
export async function PATCH(
  ...
  try {
    ...
    const productBrand = await updateProductBrand(id, body)
...
export async function DELETE(
  ...
  try {
    ...
    const { id } = await params
    const productBrand = await deleteProductBrand(id)
